...
local function drawDeveloperInfo()
    ...
    if mx and my then
        developerText(string.format("mouse: hovering %d x %d", mx, my), 0, 20, Color(0, 255, 0))
    else
        developerText(string.format("mouse: %s", gState._devInputBlocker or ""), 0, 20, Color(255, 0, 0))
    end

    local pos = gState.pos
    developerText(string.format("pos: %.2f %.2f %.2f", pos.x, pos.y, pos.z), 0, 40)
    developerText(string.format("distance %.2f / %.2f", gState._devDist or 0, gState._devHideDist or 0), 0, 53, Color(200, 200, 200, 200))
