...
function AddAllInFolder(self, folderpath, bDontinclude)
    ...
    for _, filename in pairs(files) do
        ...
        if string.find(filename, '_cl.lua') or filename == 'cl.lua' then
            if SERVER then
                AddCSLuaFile(path)
            elseif CLIENT then
                if not bDontinclude then include(path) end
                if DebugCL then print('Added CL file [' .. path .. ']') end
            end
        elseif SERVER and string.find(filename, '_sv.lua') or filename == 'sv.lua' then
            if not bDontinclude then include(path) end
            if DebugSV then print('Added SV file [' .. path .. ']') end
        else
            AddCSLuaFile(path)
            if not bDontinclude then include(path) end
            if DebugCL or DebugSV then print('Added SH file [' .. path .. ']') end
        end
    end

    for _, foldername in pairs(directories) do
        if not string.EndsWith(foldername, "_dis") then AddAllInFolder(self, folderpath .. '/' .. foldername) end
    end
end
