...
local PacketReader = PacketReader
-- https://github.com/ogniK5377/gBetterNet
...
objPacket.__tostring = function(self) return string.format('Packet("%s") %d bits', self.packet_id, self.total_bits) end
...
function objPacket.Send(self, clients)
    ...
    for _, v in pairs(self.network) do
        ...
        if v.t == dataTypes.STRING then
            ...
            for i = 1, len do
                net.WriteUInt(v.v:sub(i, i):byte(), 8)
            end
...
objPacketReader.__tostring = function(self) return 'PacketReader("' .. self.packet_id .. '")' end
